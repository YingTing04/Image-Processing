...
def compute():
  ...
  #store all computed magnitudes in image_mag
  ak = 2 * np.real(imageFT)
  bk = -2 * np.imag(imageFT)
  image_mag = np.sqrt( ak*ak + bk*bk )

  #remove DC component and find maximum magnitude
  mag_list = np.delete(image_mag.flatten(), 0)
  ...
  #remove components that are less than 40% of max
  for i in range(height):
    for j in range(width):
      if (image_mag[i][j] < (0.4 * max_mag)):
        gridImageFT[i][j] = 0
      else:
  ...
  tmp_d1 = np.sqrt((coords_translated[index_origin - 1][0] - 172) * (coords_translated[index_origin - 1][0] - 172) + (coords_translated[index_origin - 1][1] - 239) * (coords_translated[index_origin - 1][1] - 239))
  tmp_d2 = np.sqrt((coords_translated[index_origin + 1][0] - 172) * (coords_translated[index_origin + 1][0] - 172) + (coords_translated[index_origin + 1][1] - 239) * (coords_translated[index_origin + 1][1] - 239))  
  ...
  #finding unit vector with respect to angle1
  v1_x = math.cos(math.radians(angle1))
  v1_y = math.sin(math.radians(angle1))

  if ((int(v1_x) == 0) and (int(v1_y) == 0)):
    v1_x = 0
    v1_y = 0
  elif (int(v1_x) == 0):
    v1_y = 1
  ...
  #finding unit vector with respect to angle2
  v2_x = -1 * math.cos(math.radians(180 - angle2))
  v2_y = math.sin(math.radians(angle2))

  if ((int(v2_x) == 0) and (int(v2_y) == 0)):
    v2_x = 0
    v2_y = 0
  elif (int(v2_x) == 0):
    v2_y = 1
  ...
  for i in range(height):
    for j in range(width):
      ...
      if (gridImage[i][j] > 16):
        #moving in direction v1 both ways and perform wrapping for out of bounds values
        i_a1 = wrap(i + v1_y, height)
        j_a1 = wrap(j + v1_x, width)
        i_s1 = wrap(i - v1_y, height)
        j_s1 = wrap(j - v1_x, width)

        #moving in direction v2 both ways and perform wrapping for out of bounds values
        i_a2 = wrap(i + v2_y, height)
        j_a2 = wrap(j + v2_x, width)
        i_s2 = wrap(i - v2_y, height)
        j_s2 = wrap(j - v2_x, width)
        
        #calculate the mean based on points lying on the perpendicular line
        if (gridImage[i_a1][j_a1] > 16) and (gridImage[i_s1][j_s1] > 16):
          resultImage[i][j] = (gridImage[i_a2][j_a2] + gridImage[i_s2][j_s2]) / 2
        else:
          resultImage[i][j] = (gridImage[i_a1][j_a1] + gridImage[i_s1][j_s1]) / 2
